...
const _clusterWeakMap = new WeakMap();
function _readClusterId(client) {
    if (client === null || client === undefined)
        return undefined;
    const cluster = _clusterWeakMap.get(client);
    try {
...
class KafkaJsInstrumentation extends instrumentation_1.InstrumentationBase {
    ...
    init() {
        ...
        module.files.push(this._getClusterCaptureFile('kafkajs/src/producer/index.js'));
        module.files.push(this._getClusterCaptureFile('kafkajs/src/consumer/index.js'));
        return module;
    }
    ...
    _getClusterCaptureFile(path) {
        return new instrumentation_1.InstrumentationNodeModuleFile(path, ['>=0.3.0 <3'], (moduleExports) => {
            if (typeof moduleExports !== 'function')
                return moduleExports;
            const originalFactory = moduleExports;
            return function wrappedFactory(params) {
        ...
        return (original) => {
            return function run(...args) {
                ...
                if (config?.eachMessage) {
                    ...
                    instrumentation._wrap(config, 'eachMessage', instrumentation._getConsumerEachMessagePatch(this));
                }
                if (config?.eachBatch) {
                    if ((0, instrumentation_1.isWrapped)(config.eachBatch)) {
                        instrumentation._unwrap(config, 'eachBatch');
                    }
                    instrumentation._wrap(config, 'eachBatch', instrumentation._getConsumerEachBatchPatch(this));
                }
                return original.call(this, config);
            };
        };
    }
    _getConsumerEachMessagePatch(kafkaClient) {
        const instrumentation = this;
        return (original) => {
    ...
    _getConsumerEachBatchPatch(kafkaClient) {
        return (original) => {
            const instrumentation = this;
        ...
        return (original) => {
            return function transaction(...args) {
                ...
                transactionPromise
                    ...
                    const cluster = _clusterWeakMap.get(producer);
                    if (cluster !== undefined) {
                        _clusterWeakMap.set(transaction, cluster);
                    }
            ...
            return function sendBatch(...args) {
                ...
                messages.forEach(topicMessage => {
                    topicMessage.messages.forEach(message => {
                        spans.push(instrumentation._startProducerSpan(topicMessage.topic, message, this));
        ...
        return (original) => {
            return function send(...args) {
                ...
                const spans = record.messages.map(message => {
                    return instrumentation._startProducerSpan(record.topic, message, this);
                });
    ...
    _startConsumerSpan({ topic, message, operationType, ctx, link, attributes, kafkaClient, }) {
        ...
        const span = this.tracer.startSpan(`${operationName} ${topic}`, {
            ...
            attributes: {
                ...
                ...(clusterId !== undefined
                    ? { [semconv_1.ATTR_MESSAGING_KAFKA_CLUSTER_ID]: clusterId }
                    : {}),
            },
            links: link ? [link] : [],
        }, ctx);
    ...
    _startProducerSpan(topic, message, kafkaClient) {
        const clusterId = _readClusterId(kafkaClient);
