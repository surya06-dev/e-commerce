    ...
    _patchQuery(operation, tediousModule) {
        return (originalMethod) => {
            ...
            function patchedMethod(request) {
                ...
                const dbNamespace = instanceName && databaseName
                    ? `${instanceName}|${databaseName}`
                    : databaseName;
                attributes[semantic_conventions_1.ATTR_DB_NAMESPACE] = dbNamespace;
                ...
                const operationName = (0, utils_1.getOperationName)(operation);
                if (operationName !== undefined) {
                    attributes[semantic_conventions_1.ATTR_DB_OPERATION_NAME] = operationName;
                }
                ...
                if (operation === 'callProcedure' && sql) {
                    attributes[semantic_conventions_1.ATTR_DB_STORED_PROCEDURE_NAME] = sql;
                }
                ...
                const spanCollection = operation === 'callProcedure' ? sql : request.table;
                const span = thisPlugin.tracer.startSpan((0, utils_1.getSpanName)(operationName, dbNamespace, spanCollection, semantic_conventions_1.DB_SYSTEM_NAME_VALUE_MICROSOFT_SQL_SERVER), {
                ...
                const endSpan = (0, utils_1.once)((err) => {
                    ...
                    if (err) {
                        ...
                        const statusCode = err.number != null ? String(err.number) : err.code;
                        if (statusCode) {
                            span.setAttribute(semantic_conventions_1.ATTR_DB_RESPONSE_STATUS_CODE, statusCode);
                        }
                    }
                    span.end();
                });
