...
const defaultQueryStringsToRedact = Array.from(internal_types_2.DEFAULT_QUERY_STRINGS_TO_REDACT);
/**
 * Redacts sensitive query parameters from a query string (without leading '?').
 * Returns the input unchanged if it cannot be parsed.
 */
const redactQueryString = (searchParams, paramsToRedact) => {
    const params = new URLSearchParams(searchParams);
    for (const param of paramsToRedact) {
        if (params.has(param)) {
            params.set(param, internal_types_2.STR_REDACTED);
        }
    }
    return params.toString();
};
exports.redactQueryString = redactQueryString;
/**
 * Get an absolute url
 */
const getAbsoluteUrl = (requestUrl, headers, fallbackProtocol = 'http:', redactedQueryParams = Array.from(internal_types_2.DEFAULT_QUERY_STRINGS_TO_REDACT)) => {
    ...
    if (path.includes('?')) {
        try {
            ...
            const redacted = (0, exports.redactQueryString)(parsedUrl.searchParams, redactedQueryParams);
...
const getIncomingRequestAttributes = (request, options, logger) => {
    const { component, enableSyntheticSourceDetection, hookAttributes, redactedQueryParams, } = options;
    ...
    if (parsedUrl.search) {
        ...
        attributes[semantic_conventions_1.ATTR_URL_QUERY] = (0, exports.redactQueryString)(new URLSearchParams(parsedUrl.search.slice(1)), paramsToRedact);
    }
